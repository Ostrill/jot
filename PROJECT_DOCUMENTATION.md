...
```
glass_panel/          ← рабочая директория (legacy-название папки)
...
1. Пользователь вводит `$$` → вход в режим формулы. Снизу появляется стеклянная панель (`MathPreviewView`) с живым предпросмотром.
2. Пользователь печатает LaTeX — в тексте остаётся сырой `$$latex`, предпросмотр обновляется на каждый ввод.
3. Enter или стрелка-вправо в конце → формула сворачивается в `MathAttachment` (картинка inline).
4. Клик/стрелка на формулу → разворачивается обратно в `$$latex` + предпросмотр.
5. Backspace, ломающий `$$`, или стрелка-влево у начала → выход из режима (сырой текст остаётся).

#### Ключевые компоненты

- `MathEditingHost` (протокол) — `EditorTextView` через него дёргает commit/cancel из `insertNewline`/`moveRight`/`moveLeft`.
- `MathAttachment: NSTextAttachment` — хранит `latex`, держит отрендеренную картинку, центрирует её по вертикали через `attachmentBounds`.
- `MathPreviewView` — стеклянная панель предпросмотра (NSGlassEffectView + NSImageView).
- `MathRenderer.renderImage(latex:fontSize:color:)` — LaTeX → NSImage.

#### Модель состояния

- `mathEditRange: NSRange?` — диапазон редактируемого `$$…` (nil = не в режиме).
- `isProcessingMath` — guard от реентрантности при программных правках (commit/expand меняют selection → ловится в `textViewDidChangeSelection`).
- Диапазон поддерживается в `textView(_:shouldChangeTextIn:)` (растёт/сжимается с правками).
- Детект `$$` и валидация — в `textDidChange` → `handleMathAfterTextChange`.
- Переходы commit/cancel/expand — в `textViewDidChangeSelection`.

#### Round-trip (сохранение/загрузка)

- `text` getter → `serializedText()`: формулы-attachment сериализуются обратно в `$$latex$$` (стандартный display-math). Без потери данных.
- `setText` → `attributedStringRenderingFormulas`: при загрузке `$$…$$` парсятся обратно в attachment'ы.

#### Известные ограничения v1

- Формула считается «от `$$` до каретки» при редактировании; правка в середине уже набранной формулы с последующим коммитом может вести себя неидеально (caret-as-end модель).
- При смене размера шрифта уже отрендеренные формулы не перерисовываются (остаются в старом размере).
- Клик строго по правой половине свёрнутой формулы может не открыть редактирование (проверяется символ справа от каретки). Клик по левой половине / стрелка-вправо на формулу — работают.
- Двухслойный текст: `backdropTextView` получает `U+FFFC` на месте формул (рендерится невидимо). Цветной задний слой под формулами не активен — это ожидаемо.
