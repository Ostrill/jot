...
Кнопки (close/minimize/zoom) **позиционирует сам AppKit**. Из-за большого скругления (radius 30) на дефолтной позиции (9, 9 от верхне-левого угла) кнопки некрасиво жмутся к дуге. Чтобы AppKit поставил их ниже и с бо́льшим отступом — окну выдаётся **пустой прозрачный unified toolbar** (`installTitlebarToolbar(in:)`: `window.toolbar = NSToolbar(...)`, `window.toolbarStyle = .unified`). Это поднимает высоту титлбара (с 32 до 66), и AppKit сам опускает кнопки до (19, 19) — центр кнопки (26, 26) уже вне дуги радиуса 30. Тулбар пустой и невидимый: область титлбара перекрыта нашим `fullSizeContentView`-стеклом.

### Почему так, а не ручной сдвиг и не accessory

- **Ручной `setFrameOrigin`** ломает hover: символы (×/−/+) рисует `NSThemeFrame` по своей tracking-зоне, которая остаётся на дефолтном месте → зона наведения уезжает от кнопок (проверено: при сдвиге вправо на 10px hover-зона оказывалась на полкнопки левее). Поэтому кнопки нельзя двигать самим.
- **Titlebar-accessory** (старый `installTitlebarSpacer`) **не работал**: замер показал, что accessory высоты N не увеличивает титлбар (оставался 32, кнопки на 9,9). Удалён. Не возвращать.
- **Unified toolbar** увеличивает титлбар «легально», и кнопки двигает сам AppKit → tracking-зона едет вместе с ними, hover цел.

`positionTrafficLights()` не двигает кнопки, а только дёргает `editorView.needsLayout` (статус-блок справа сверху следует за фактической позицией кнопок). `titleBarMetrics()` читает **фактический** фрейм close-кнопки (`buttonSuperview.convert(...)`), поэтому статус и верхний отступ редактора подстраиваются автоматически.

### Тонкая настройка

И горизонталь, и вертикаль кнопок задаёт AppKit; напрямую публично не двигаются. Чтобы опустить кнопки ещё ниже / дальше от края — увеличить высоту тулбара (добавить невидимый высокий toolbar-item). Пустой unified toolbar даёт (19, 19).
